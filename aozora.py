...
def load_data():
    """データの読み込み。"""
    df_train = pd.read_csv('https://raw.githubusercontent.com/ozt-ca/tjo.hatenablog.samples/master/r_samples/public_lib/jp/aozora/aozora_8writers_train.csv', header=None, names=['text', 'class'])
    df_test = pd.read_csv('https://raw.githubusercontent.com/ozt-ca/tjo.hatenablog.samples/master/r_samples/public_lib/jp/aozora/aozora_8writers_test.csv', header=None, names=['text', 'class'])
    ...
    train_dataset = MyDataset(X_train, y_train, NUM_CLASSES, INPUT_SHAPE, data_augmentation=True)
...
def create_model():
    ...
    x = tk.keras.layers.Dense(NUM_CLASSES, kernel_regularizer=tk.keras.regularizers.l2(1e-4), activation='softmax')(x)
...
class MyDataset(tk.data.Dataset):
    ...
    def __getitem__(self, index):
        X = np.frombuffer(self.X[index].replace(' ', '').encode('utf-16-le'), dtype=np.uint16)
